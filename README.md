...
`TIMELESS_ADMIN_PASSWORD` 只用于第一次创建管理员账号。初始化后请在“账号管理”中修改密码，之后管理员密码以数据库中的值为准，重启服务不会被环境变量覆盖。
...
后台“账号管理”支持开放注册、邀请码注册和关闭注册三种模式。邀请码以加密摘要保存。管理员还可以停用或启用普通账号、重置普通用户密码、删除账号及其 Todo，并修改管理员自己的登录密码。

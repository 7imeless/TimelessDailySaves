...
def article_cards(posts: list[sqlite3.Row]) -> str:
    ...
    for index, post in enumerate(posts, start=1):
        ...
        cards.append(
            f"""
          <article class="article-entry{featured}">
...
def render_todo_panel(user: sqlite3.Row | None, token: str | None, selected_date: date, edit_todo: sqlite3.Row | None = None, message: str = "") -> str:
    ...
    past = [todo for todo in items if todo["due_date"] < today.isoformat() and todo["due_date"] != selected_value]
    ...
    return f'''<section class="todo-panel" id="todo">
      ...
      <details class="todo-history"><summary>过去的 Todo <span>{len(past):02d}</span></summary><ul class="todo-list">{todo_list_items(past, csrf_for(token), '还没有过去的待办。', selected_date)}</ul></details>
        ...
        if path == "/admin/save":
            ...
            with db_connection() as connection:
                ...
                if article_id is None:
                    connection.execute(
                        "INSERT INTO articles (title, slug, category, excerpt, content, status, created_at, updated_at, published_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                        (title, slug, category, excerpt, content, status, now, now, now if status == "published" else None),
                    )
                else:
                    ...
                    connection.execute(
                        "UPDATE articles SET title = ?, slug = ?, category = ?, excerpt = ?, content = ?, status = ?, updated_at = ?, published_at = ? WHERE id = ?",
                        (title, slug, category, excerpt, content, status, now, published_at, article_id),
                    )
